...
class Triangle2DTest {
	...
	public Point2D[] points_copy = new Point2D[3];
	...
	public Triangle2D _triangle = new Triangle2D(points);
	public Triangle2D _copy = new Triangle2D(points_copy);


	@BeforeEach
	public void hadas() {
		set_array();
		_triangle = new Triangle2D(points);
		_copy = new Triangle2D(points_copy);
	}
	...
	@Test
	void Triangle2D_supertest() {
		set_array();
		Triangle2D x = new Triangle2D(points);

		assertTrue(x instanceof Triangle2D);
	}
...
//		assertTrue(this._triangle.contains(p1));
	...
	void move_supertest() {
		...
		movelooper(vec1);
		Triangle2D move1 =new Triangle2D(points); hadas();
		movelooper(vec2);
		Triangle2D move2 =new Triangle2D(points); hadas();
		movelooper(vec3);
		Triangle2D move3 =new Triangle2D(points); hadas();

		this._triangle.move(vec1);
		assertEquals(_triangle.toString(), move1.toString()); hadas(); hadas();
		this._triangle.move(vec2);
		assertEquals(_triangle.toString(),move2.toString()); hadas();
		this._triangle.move(vec3);
		assertEquals(_triangle.toString(),move3.toString()); hadas();
	}
	...
	void scale_supertest() {
		...
		_triangle.scale(cen1, ratio3);
		assertEquals(_copy.perimeter(), _triangle.perimeter(),0.05);				hadas();
		_triangle.scale(cen1, ratio4);
		assertEquals(_copy.perimeter(), _triangle.perimeter(),0.05);				hadas();

		_triangle.scale(cen2, ratio1);
		assertEquals(_copy.perimeter()*ratio1, _triangle.perimeter(),0.05);		hadas();		
		_triangle.scale(cen2, ratio2);
		assertEquals(_copy.perimeter(), _triangle.perimeter(),0.05);				hadas();
		System.out.println(_copy.getPoints()[0].toString());///////////////////////////////////////////
		_triangle.scale(cen2, ratio3);
		System.out.println(_copy.getPoints()[0].toString());
		assertEquals(_copy.perimeter()*ratio3, _triangle.perimeter(),0.05);		hadas();
		_triangle.scale(cen2, ratio4);
		assertEquals(_copy.perimeter()*ratio4, _triangle.perimeter(),0.05);		hadas();

		_triangle.scale(cen3, ratio1);
		assertEquals(_copy.perimeter()*ratio1, _triangle.perimeter(),0.05);		hadas();
		_triangle.scale(cen3, ratio2);
		assertEquals(_copy.perimeter(), _triangle.perimeter(),0.05);			hadas();
		_triangle.scale(cen3, ratio3);
		assertEquals(_copy.perimeter()*ratio3, _triangle.perimeter(),0.05);		hadas();
		_triangle.scale(cen3, ratio4);
		assertEquals(_copy.perimeter()*ratio4, _triangle.perimeter(),0.05);		hadas();
	}
	...
	void rotate_supertest() {
		...
		_triangle.rotate(cen1, angleDegrees3);
		rotatelooper(cen1, angleDegrees3);
		assertEquals(_copy.toString(), _triangle.toString()); hadas();
		_triangle.rotate(cen1, angleDegrees4);
		rotatelooper(cen1, angleDegrees4);
		assertEquals(_copy.toString(), _triangle.toString()); hadas();

		_triangle.rotate(cen2, angleDegrees1);
		rotatelooper(cen1, angleDegrees3);
		assertEquals(_copy.toString(), _triangle.toString()); hadas();
		_triangle.rotate(cen2, angleDegrees2);
		rotatelooper(cen2, angleDegrees2);
		assertEquals(_copy.toString(), _triangle.toString()); hadas();   
		_triangle.rotate(cen2, angleDegrees3);
		rotatelooper(cen2, angleDegrees3);
		assertEquals(_copy.toString(), _triangle.toString()); hadas();
		_triangle.rotate(cen2, angleDegrees4);
		rotatelooper(cen2, angleDegrees4);
		assertEquals(_copy.toString(), _triangle.toString()); hadas();

		_triangle.rotate(cen3, angleDegrees1);
		rotatelooper(cen3, angleDegrees1);
		assertEquals(_copy.toString(), _triangle.toString()); hadas();
		_triangle.rotate(cen3, angleDegrees2);
		rotatelooper(cen3, angleDegrees2);
		assertEquals(_copy.toString(), _triangle.toString()); hadas();
		_triangle.rotate(cen3, angleDegrees3);
		rotatelooper(cen3, angleDegrees3);
		assertEquals(_copy.toString(), _triangle.toString()); hadas();
		_triangle.rotate(cen3, angleDegrees4);
	...
	void getPoints_supertest() {
		Triangle2D copy = new Triangle2D(points);
