...
public class Point2D{
	...
	// changes the distance if the point to the cen by the ratio
	public void scale(Point2D cen, double ratio) {
		if (cen == null || ratio == 1 ||ratio == 0) {    // invalid inputs stop
			System.err.println("invalid input");
			return;
			}
			this.move(new Point2D((this._x-cen.x())*(ratio-1),(this._y-cen.y())*(ratio-1)));
	}
	
	// rotate the point by the angle and the cen point
	public void rotate(Point2D cen, double angleDegrees) {
		if (cen == null ) {    // invalid inputs stop
			System.err.println("invalid input");
			return;
			}
		double rad = Math.toRadians(angleDegrees);
		double newX = cen.x() + ( Math.cos(rad) * (this.x()-cen.x()) + Math.sin(rad) * (this.y() -cen.y()));
		double newy = cen.y() + ( -Math.sin(rad) * (this.x()-cen.x()) + Math.cos(rad) * (this.y() -cen.y()));
		this._x =newX;
		this._y =newy;
	}
